...
if __name__ == '__main__':
    ...
    g = zarr.open_group(store_segment, mode = 'r')
    ...
    if os.path.exists(IN_FILENAME[:-5] + '_tracks.h5'):
        ...
        objects = btrack.utils.segmentation_to_objects(
            halfres[:,DATA_CHANNEL,:, :, :],
            properties=('centroid', 'area')
        )
        print("Starting btrack")
        tracks, refs, napari_data = runBtrackOnZarrStore(fullres.shape, objects)
